...
begin
  ...
  begin
    ...
    ea[0].grfAccessPermissions := DELETE or DESKTOP_ENUMERATE or READ_CONTROL or WRITE_DAC or
      WRITE_OWNER;
...
var
  ...
  cSecureViewerSessionID: Cardinal;
  hSnapshot: THandle;
...
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Log('Starting to terminate processes with threads on secure desktop');
          cSecureViewerSessionID := WTSGetActiveConsoleSessionId;
          hSnapshot := CreateToolhelp32Snapshot(TH32CS_SNAPTHREAD, 0);
          ThreadEntry.dwSize := SizeOf(TThreadEntry32);
          if Thread32First(hSnapshot, ThreadEntry) then
          begin
            repeat
              cProcessID := ThreadEntry.th32OwnerProcessID;
              if ProcessIdToSessionId(cProcessId, cSessionID) then
              begin
                if (cSessionID = cSecureViewerSessionID) then
                begin
                  cThreadID := ThreadEntry.th32ThreadID;
                  hThreadDesktop := GetThreadDesktop(cThreadID);
                  dwLengthNeeded := 0;
                  Windows.GetUserObjectInformation(hThreadDesktop, UOI_NAME, nil, 0, dwLengthNeeded);
                  szDesktopName := nil;
                  GetMem(szDesktopName, dwLengthNeeded);
                  Windows.GetUserObjectInformation(hThreadDesktop, UOI_NAME, szDesktopName, dwLengthNeeded, dwLengthNeeded);
                  if (StrPas(szDesktopName) = StrPas(SecureViewerDesktopName)) then
                  begin
                    Log(Format('   PID: %.6d  TID: %.6d  Session %.2d  HDESK %.8d',[cProcessID, cThreadID, cSessionID, hThreadDesktop]));
                  end
                  else
                  begin
                    Log(Format('   %.5d  %s  %s', [cProcessID, StrPas(szDesktopName), StrPas(SecureViewerDesktopName)]));
                  end;
                  FreeMem(szDesktopName);
                end;
              end;
            until (not Thread32Next(hSnapshot, ThreadEntry));
          end;
          CloseHandle(hSnapshot);
          Log('Completed terminating processes with threads on secure desktop');
  ...
  // Secure viewer process group (i.e. capability) is removed separately from system
  // Create new group
...
begin
  ...
  if WTSQueryUserToken(UserSessionId, hUserDesktopProcessToken) then
  begin
    Log('Token for user in active console session');
