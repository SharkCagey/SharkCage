...
uses
  ...
  Dialogs, ExtCtrls, ShellApi, StdCtrls, InvokeRegistry, Rio, SOAPHTTPClient;
...
type
  TfmAppInfo = class(TForm)
    ...
    procedure btnHideAppClick(Sender: TObject);
  private
...
uses
  PNGImage, SDCommon, dmScreenshot, SDSecureDisplaySvcIntf, SDProtocol;
...
var
  nLblApplicationNameBottom: Integer;
begin
  ...
  btnHideApp.Caption := 'Close';
  btnHideApp.Left := lblApplicationName.Left;
  btnHideApp.Width := lblApplicationName.Width;
  nLblApplicationNameBottom := lblApplicationName.Top + lblApplicationName.Height;
  btnHideApp.Top := nLblApplicationNameBottom +
    ((panelAppInfo.Height - nLblApplicationNameBottom - btnHideApp.Height) div 2);
  btnHideApp.Hide;
...
procedure TfmAppInfo.btnHideAppClick(Sender: TObject);
var
  cResponse: Cardinal;
begin
  ...
  try
    cResponse :=
      (ConsumeWebService as ISecureDisplaySvc).returnFromApplication(ApplicationKey);
    if (cResponse = SDP_SOAP_RESPONSE_RETURNED_FROM_APP) then
    begin
      Log(Format('Request sent to return from app %d.', [ApplicationKey]));
    end;
  except
    Log(Format('Request to return from app %d failed.', [ApplicationKey]));
  end;
end;
